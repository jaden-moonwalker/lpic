...
# Basics
## 文件系统层次结构标准 File Hierarchy Standard (FHS)
| Path   | Content                             |
| ------ | ----------------------------------- |
| /bin   | 单用户模式下必要命令（可执行文件）；面向所有用户，例如： cat、 ls、 cp。|
...
| /usr   | 用于存储只读用户数据的第二层次； 包含绝大多数的(多)用户工具和应用程序。第三方程序。                  |
| /proc  | 虚拟文件系统，将内核与进程状态归档为文本文件。例如：uptime、 network。在Linux中，对应Procfs格式挂载。                  |
| /sys   | 虚拟文件系统                  |
| /mnt   | 挂载其他文件系统(ext4、xfs、ntfs等)      |
| /media | 挂载可移动媒体设备(光驱、外置硬盘、U盘等)      |
| /home  | 存储每个用户的个人文件和设置。                          |
| /run   | 存储系统和服务的运行时信息(进程ID、套接字等)。      |
...
| `cd`    | `-`              | 返回上一次所在的目录                              |
|         | `~`              | 切换到当前用户的home目录                               |
|         | `~username`      | 切换到指定用户的home目录                |
| `pwd`   |                  | Print working dir, 显示当前用户所在的工作目录的路径                                 |
| `ls`    |                  | 列出当前目录下的所有文件和目录                                 |
|         | `-l`             | 以长格式列出当前目录下的所有文件和目录，包括权限、文件大小、最后修改时间等信息                                    |
|         | `-a`             | 列出当前目录下的所有文件和目录，包括隐藏文件 (`-A` 排除 `.` 和 `..`)     |
|         | `-r`             | 用来将文件以反向顺序显示，即先显示最后修改的文件。|
|         | `-R`             | 递归地列出当前目录下的所有文件和目录，包括子目录下的文件和目录                                           |
|         | `-S`             | 选项用来将文件按照大小排序，而不是按照名字排序。                                     |
|         | `-t`             | 选项用来将文件按照修改时间排序，而不是按照名字排序。                             |
| `mkdir` |                  | 创建一个名为目录名的目录                           |
|         | `-p`             | 创建路径名指定的目录，如果中间的目录不存在，则会一并创建。也可以忽略目录已存在的错误。                           |
| `cp`    | `-r`             | 复制目录时递归复制子目录。                                          |
|         | `-i`             | 在覆盖文件时会先询问用户。                                          |
| `rmdir` | `-p`             | rmdir命令只能删除空目录，如果目录中有文件或者子目录，则会报错。<br>如果要删除非空目录，可以使用rm命令，例如"rm -r mydir"就是删除名为mydir的目录及其所有子目录和文件。                      |
| `rm`    |                  | 删除指定的文件 |
|         | `-r`             | 删除指定的目录及其内容 |
|         | `-R`             | 递归地删除指定的目录及其中的所有内容 |
|         | `-r`             | -r 选项与 -R 选项的功能是相同的。不过，一般来说，-R 选项被认为是更常用的写法，<br> 而 -r 选项可能是为了与其他系统（例如 Mac OS）保持兼容而提供的。 |
|         | `-i`             | 在删除文件时会先询问用户是否确定 |
|         | `-f`             | 强制删除文件，不会询问用户确认 |
|         | `-Rf`            | 强制删除目录及其内容，不会询问用户确认 |
| `mv`    |                  | 移动或重命名文件或目录                                  |
|         | `-f`             | 强制执行                                  |
|         | `-i`             | 询问用户是否覆盖已有的目标文件                                  |
|         | `-n`             | 不覆盖已有的目标文件                                  |
|         | `-v`             | 显示详细信息                                  |
| `find`  |                  | 用于查找文件或目录                  |
|         | `-name pattern` | 指定要查找的文件或目录名。                  |
|         | `-iname pattern` | 指定要查找的文件或目录名。忽略大小写。                  |
|         | `-mmin n`        | 指定要查找的文件或目录的最近修改时间，单位是分钟。<br> 例如，如果你想查找最近修改时间超过1小时的文件，可以使用"-mmin +60"。                       |
|         | `-mtime n`       | 指定要查找的文件或目录的最近修改时间。                          |
|         | `-regex pattern` | 使用正则表达式来指定要查找的文件或目录名。                              |
|         | `-size n[kMG]`   | 指定要查找的文件或目录的大小。<br>例如，如果你想查找大小超过1KB的文件，可以使用"-size +1K"。 (`-n` 小于; `+n` 大于)  |
|         | `! searchparams` | 取反查找。如果你想查找所有不是以".txt"为扩展名的文件，可以使用"-name ".txt" -a ! -name ".txt"<br>（注意，这里使用了"-a"选项表示"与"的意思）。                                     |
